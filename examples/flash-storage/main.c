...
int main()
{
  ...
  if (ndn_platform_flash_status() == NDN_PLAT_FLASH_READY)
  {
    // Check If Existing Storage Matches
    uint8_t buffer_data[sizeof(data)];
    uint8_t buffer_string[strlen(string)];
    ndn_platform_flash_read(address, buffer_data, sizeof(data));
    ndn_platform_flash_read(address + sizeof(data), buffer_string, strlen(string));
    ...
    if (r1 == 0 && r2 == 0)
    {
      ...
      ndn_platform_flash_erase(address);
    }
    else
    {
      // Check Usable Flash Space and Prepare to Write
      printf("Usable Flash Size: %d bytes\n", (int)ndn_platform_flash_get());
      printf("Write Flash with Data and String\n");

      // Erase Page and Write
      ndn_platform_flash_erase(address);
      ndn_platform_flash_write(address, (uint8_t*)&data, sizeof(data));
      ndn_platform_flash_write(address + sizeof(data), (uint8_t*)string, strlen(string));
